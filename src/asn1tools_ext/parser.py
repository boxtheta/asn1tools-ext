...
class Tokens(object):
    ...
    def __repr__(self):
        return "Tokens(tag='{}', tokens='{}')".format(self.tag, self.tokens)
...
def convert_size(tokens):
    ...
    if tokens[0] == "SIZE":
        ...
        for item_tokens in tokens[1].asList():
            if ".." in item_tokens:
                value = (convert_number(item_tokens[0]), convert_number(item_tokens[2]))
            else:
            ...
            if item_number in used_numbers:
                raise ParseError(
                    "Duplicated ENUMERATED number {} at line {}.".format(
                        item_number, lineno(location, string)
                    )
                )
...
def convert_tag(tokens):
    if len(tokens) > 0:
        if len(tokens[0]) == 1:
            tag = {"number": int(tokens[0][0])}
        else:
            tag = {"number": convert_number(tokens[0][1]), "class": tokens[0][0]}
...
def convert_members(tokens):
    ...
    for member_tokens in tokens:
        ...
        if member_tokens[0] == "COMPONENTS OF":
            members.append({"components-of": member_tokens[1][0]["type"]})
            continue
...
def convert_sequence_type(_s, _l, tokens):
    return {"type": "SEQUENCE", "members": convert_members(tokens[2])}
...
def convert_set_type(_s, _l, tokens):
    return {"type": "SET", "members": convert_members(tokens[2])}
...
def convert_choice_type(_s, _l, tokens):
    return {"type": "CHOICE", "members": convert_members(tokens[2])}


def convert_defined_type(_s, _l, tokens):
    converted = {"type": tokens[0]}
...
def convert_integer_type(_s, _l, tokens):
    ...
    if tokens[1]:
        compiled["named-numbers"] = {item[0]: item[2] for item in tokens[1]}
...
def convert_ia5_string_type(_s, _l, _tokens):
    return {"type": "IA5String"}


def convert_any_defined_by_type(_s, _l, tokens):
    return {"type": "ANY DEFINED BY", "value": tokens[1], "choices": {}}
...
def convert_parameterized_object_set_assignment(_s, _l, tokens):
    ...
    converted_type = {"class": tokens[1], "members": members}

    return ("parameterized-object-set-assignment", tokens[0], converted_type)


def convert_parameterized_object_assignment(_s, _l, tokens):
    type_ = tokens[1]

    converted_type = {"type": type_, "value": None}

    return ("parameterized-object-assignment", tokens[0], converted_type)
...
def convert_parameterized_object_class_assignment(_s, _l, tokens):
    ...
    return ("parameterized-object-class-assignment", tokens[0], converted_type)
...
def convert_parameterized_type_assignment(_s, _l, tokens):
    ...
    return ("parameterized-type-assignment", tokens[0], converted_type)
...
def convert_parameterized_value_assignment(_s, _l, tokens):
    ...
    converted_type = {"type": type_, "value": convert_value(tokens[2], type_)}

    return ("parameterized-value-assignment", tokens[0], converted_type)
...
def convert_assignment_list(_s, _l, tokens):
    ...
    for kind, name, value in tokens:
        if kind == "parameterized-object-set-assignment":
            ...
            raise InternalParserError("Unrecognized assignment kind {}.".format(kind))
...
def create_grammar():
    ...
    #       SEQUENCE/SET OF.
    size_paren = (
        Suppress(Optional(left_parenthesis))
        + size_constraint
    ...
    tag = Group(
        Optional(
            Suppress(left_bracket)
            - Group(Optional(UNIVERSAL | APPLICATION | PRIVATE) + class_number)
    ...
    parameter_list = Group(
        Optional(
            Suppress(left_brace) + delimitedList(parameter) + Suppress(right_brace)
        )
    )

    # X.683: 9. Referencing parameterized definitions
    actual_parameter = Group(
        type_ | value | value_set | defined_object_class | object_ | object_set
    )
    actual_parameter_list = Group(
        Suppress(left_brace) + delimitedList(actual_parameter) + Suppress(right_brace)
    )
    ...
    parameterized_value_set_type = simple_defined_type + actual_parameter_list
    simple_defined_value = external_value_reference | value_reference
    parameterized_value = simple_defined_value + actual_parameter_list
    simple_defined_type <<= external_type_reference | type_reference
    parameterized_type = simple_defined_type + actual_parameter_list
    ...
    at_notation = Suppress(at) - (
        component_id_list | Combine(level + component_id_list)
    )
    ...
    table_constraint = component_relation_constraint | simple_table_constraint
    ...
    general_constraint = (
        user_defined_constraint | table_constraint | contents_constraint
    )
    ...
    object_class_reference = NotAny(reserved_words) + Regex(r"[A-Z][A-Z0-9-]*")
    ...
    defined_object_set <<= external_object_set_reference | object_set_reference
    ...
    type_field_spec = type_field_reference + Optional(OPTIONAL | (DEFAULT - type_))
    field_spec = Group(
        type_field_spec
        | fixed_type_value_field_spec
    ...
    parameterized_object_class_assignment = (
        object_class_reference + Suppress(parameter_list) + assign + object_class
    )

    # X.681: 10. Syntax list
    literal = word | comma
    required_token = literal | primitive_field_name
    optional_group = left_bracket + OneOrMore(token_or_group_spec) + right_bracket
    token_or_group_spec <<= required_token | optional_group
    syntax_list <<= left_brace + OneOrMore(token_or_group_spec) + right_brace
    ...
    default_syntax = (
        Suppress(left_brace) + delimitedList(field_setting) + Suppress(right_brace)
    )
    ...
    object_ <<= defined_object | object_defn | object_from_object | parameterized_object
    ...
    object_set_spec = (
        ...
        + Optional(comma + ellipsis + Optional(comma + additional_element_set_spec))
    ...
    object_class_field_value = open_type_field_val | fixed_type_field_val
    object_class_field_type = Combine(defined_object_class + dot + field_name)
    ...
    exception_spec = Optional(
        exclamation_mark + NoMatch().setName('"exceptionSpec" not implemented')
    )
    ...
    component_constraint = Optional(value_constraint) + Optional(presence_constraint)
    ...
    partial_specification = (
        left_brace + Group(ellipsis + Suppress(comma) + type_constraints) + right_brace
    )
    ...
    value_range = (
        (Combine(integer + dot) + Suppress(range_separator))
    ...
    element_set_specs = root_element_set_spec + Optional(
        Suppress(comma)
        - ellipsis
        + Optional(Suppress(comma) - additional_element_set_spec)
    )

    # X.680: 45. Constrained types
    subtype_constraint = element_set_specs
    constraint_spec = general_constraint | subtype_constraint
    constraint_spec.setName("one or more constraints")
    constraint <<= (
        Suppress(left_parenthesis) - constraint_spec - Suppress(right_parenthesis)
    )
    ...
    cell = number
    quadruple = (
        left_brace + group + comma + plane + comma + row + comma + cell + right_brace
    )
    table_column = number
    table_row = number
    ...
    restricted_character_string_value = (
        cstring | character_string_list | quadruple | tuple_
    )
    ...
    character_string_value = (
        restricted_character_string_value | unrestricted_character_string_value
    )
    character_string_type = (
        restricted_character_string_type | unrestricted_character_string_type
    )
    ...
    relative_oid_components = Group(number_form | name_and_number_form | defined_value)
    relative_oid_component_list = OneOrMore(relative_oid_components)
    relative_oid_value = (
        Suppress(left_brace) + relative_oid_component_list + Suppress(right_brace)
    )
    ...
    obj_id_components = Group(
        name_and_number_form | defined_value | number_form | name_form
    )
    obj_id_components_list = OneOrMore(obj_id_components)
    object_identifier_value = (
        Suppress(left_brace) + obj_id_components_list + Suppress(right_brace)
    ) | (
        Suppress(left_brace)
        + defined_value
    ...
    object_identifier_type = OBJECT_IDENTIFIER + Optional(
        left_parenthesis + delimitedList(word, delim="|") + right_parenthesis
    )
    ...
    extension_addition_alternatives_group = Group(
        left_version_brackets
        + Suppress(version_number)
        - Group(alternative_type_list)
        - right_version_brackets
    )
    extension_addition_alternative = extension_addition_alternatives_group | named_type
    ...
    extension_addition_alternatives = Optional(
        Suppress(comma) + extension_addition_alternatives_list
    )
    root_alternative_type_list = alternative_type_list
    alternative_type_lists = root_alternative_type_list + Optional(
        Suppress(comma)
        + extension_and_exception
        + extension_addition_alternatives
        + optional_extension_marker
    )
    choice_type = CHOICE - left_brace + Group(alternative_type_lists) - right_brace
    ...
    set_type = (
        ...
        + Group(
            Optional(
                ...
                | (extension_and_exception + optional_extension_marker)
            )
        )
        - right_brace
    )
    ...
    sequence_value <<= left_brace + Optional(component_value_list) + right_brace
    component_type = Group(
        named_type + Group(Optional(OPTIONAL | (DEFAULT + value)))
        | (COMPONENTS_OF - type_)
    )
    version_number <<= Optional(number + Suppress(colon))
    extension_addition_group = Group(
        left_version_brackets
        + Suppress(version_number)
    ...
    component_type_lists <<= (
        ...
        + Optional(
            ...
            + extension_additions
            + (
                (extension_end_marker + Suppress(comma) + root_component_type_list)
                | optional_extension_marker
            )
        )
    ) | (
    ...
    bit_string_type = BIT_STRING + Group(
        Optional(
            Suppress(left_brace)
            + delimitedList(
                Group(
    ...
    bit_string_value = Tag(
        "BitStringValue",
        bstring
        | hstring
        | Tag(
            "IdentifierList",
            Suppress(left_brace) + Optional(identifier_list) + Suppress(right_brace),
        )
        | (CONTAINING - value),
    )

    # X.680: 20. Notation for the real type
    special_real_value = PLUS_INFINITY | MINUS_INFINITY
    numeric_real_value = real_number | sequence_value
    ...
    enumerations = Group(
        Group(root_enumeration)
        + Group(
            Optional(
                Group(Suppress(comma - ellipsis + exception_spec))
                + Optional(Suppress(comma) - additional_enumeration)
            )
        )
    )
    enumerated_type = ENUMERATED - left_brace + enumerations - right_brace
    ...
    integer_type = INTEGER + Group(
        Optional(Suppress(left_brace) + named_number_list + Suppress(right_brace))
    )
    ...
    # | referenced_value
    # | builtin_value)
    named_type <<= Group(identifier - tag - type_)
    ...
    external_value_reference <<= module_reference + dot + value_reference
    external_type_reference <<= module_reference + dot + type_reference
    defined_type <<= (
    ...
    defined_value <<= external_value_reference | parameterized_value | value_reference

    # X.680: 12. Module definition
    module_reference <<= NotAny(reserved_words) + Regex(r"[A-Z][a-zA-Z0-9-]*").setName(
        "modulereference"
    )
    assigned_identifier = Suppress(
        Optional(object_identifier_value | (defined_value + ~(comma | FROM)))
    )
    ...
    symbol = parameterized_reference | reference
    symbol_list = Group(delimitedList(symbol))
    symbols_from_module = (
        symbol_list
        + FROM
        + global_module_reference
        + Suppress(Optional(WITH_SUCCESSORS | WITH_DESCENDANTS))
    )
    symbols_imported = ZeroOrMore(Group(symbols_from_module))
    imports = Optional(Suppress(IMPORTS) - symbols_imported - Suppress(semi_colon))
    symbols_exported = OneOrMore(symbol_list)
    exports = Suppress(Optional(EXPORTS - (ALL | symbols_exported) + semi_colon))
    assignment = (
        parameterized_object_set_assignment
        | parameterized_object_assignment
        | parameterized_object_class_assignment
    ...
    definitive_obj_id_component = Group(
    ...
    parameterized_type_assignment.setParseAction(convert_parameterized_type_assignment)
...
def ignore_comments(string):
    ...
    comments = [
        (mo.start(), mo.group(0)) for mo in re.finditer(r"(/\*|\*/|--|\n)", string)
    ]
    ...
    if in_single_line_comment:
        raise ParseSyntaxException(
            string, start_offset, "Missing newline or -- for single line comment"
        )

    if multi_line_comment_depth > 0:
        raise ParseSyntaxException(
            string, start_offset, "Missing */ for multi line comment"
        )
...
def parse_string(string):
    ...
    try:
        ...
        raise ParseError(
            "Invalid ASN.1 syntax at line {}, column {}: '{}': {}.".format(
                e.lineno, e.column, e.markInputline(), e.msg
            )
        )

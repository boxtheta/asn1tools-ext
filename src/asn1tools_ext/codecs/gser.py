...
class MembersType(Type):
    ...
    def encode(self, data, separator, indent):
        ...
        for member in self.members:
            ...
            if name in data:
                try:
                    encoded_member = member.encode(data[name], member_separator, indent)
                except ErrorWithLocation as e:
                    # Add member location
                    e.add_location(member)
                    raise e

                encoded_member = "{}{} {}".format(
                    member_separator, member.name, encoded_member
                )
...
class ArrayType(Type):
    ...
    def encode(self, data, separator, indent):
        ...
        for entry in data:
            encoded_element = self.element_type.encode(entry, element_separator, indent)
            encoded_element = "{}{}".format(element_separator, encoded_element)
...
class Enumerated(Type):
    def __init__(self, name, values, numeric):
        ...
        if numeric:
            ...
            self.data_to_value = {v: v for v in enum_values_as_dict(values).values()}
...
class SequenceOf(ArrayType):
    def __init__(self, name, element_type):
        super(SequenceOf, self).__init__(name, "SEQUENCE OF", element_type)
...
class SetOf(ArrayType):
    def __init__(self, name, element_type):
        super(SetOf, self).__init__(name, "SET OF", element_type)
...
class DateTime(Type):
    ...
    def encode(self, data, _separator, _indent):
        return '"{}"'.format(str(data).replace(" ", "T"))
...
class CompiledType(compiler.CompiledType):
    ...
    def encode(self, data, indent=None):
        ...
        encoded = "{} {} ::= {}".format(
            self._value_name, self._value_type, encoded.lstrip(" ")
        )
...
class Compiler(compiler.Compiler):
    ...
    def compile_type(self, name, type_descriptor, module_name):
        ...
        if type_name == "SEQUENCE":
            members, _ = self.compile_members(type_descriptor["members"], module_name)

...
from .compiler import clean_bit_string_value, enum_values_as_dict
...
class MissingMandatoryFieldError(DecodeError):
    ...
    def __init__(self, member, offset):
        super().__init__(
            "{} is missing and has no default value".format(member),
            offset=offset,
            location=member,
        )
...
class DecodeTagError(DecodeError):
    ...
    def __init__(self, asn_type, data, offset, location=None):
        ...
        """
        self.actual_tag = format_bytes(
            read_tag(data, offset)
...
def decode_length(encoded, offset, enforce_definite=True):
    ...
    try:
        ...
        raise OutOfByteDataError(
            "Ran out of data when trying to read length", offset=offset
        )
    ...
    if length & 0x80:  # Faster than > 127
        ...
        if length == 128:
            if enforce_definite:
                raise DecodeError(
                    "Expected definite length, but got indefinite.", offset=offset - 1
                )
    ...
    if offset + length > data_length:
        raise MissingDataError(
            "Expected at least {} contents byte(s), but got {}.".format(
...
def skip_tag(data, offset):
    try:
        ...
        raise OutOfByteDataError("Ran out of data when reading tag", offset=offset)
...
def encode_real(data):
    if data == float("inf"):
        ...
        if -129 < exponent < 128:
            ...
            raise NotImplementedError("REAL exponent {} out of range.".format(exponent))
...
def encode_object_identifier(data):
    ...
    encoded_subidentifiers = encode_object_identifier_subidentifier(first_subidentifier)

    for identifier in identifiers[2:]:
        encoded_subidentifiers += encode_object_identifier_subidentifier(identifier)
...
def decode_object_identifier(data, offset, end_offset):
    subidentifier, offset = decode_object_identifier_subidentifier(data, offset)
...
class StandardDecodeMixin(object):
    ...
    indefinite_allowed = (
        False  # Whether indefinite length encoding is allowed for this type
    )
    ...
    def decode(self, data, offset, values=None):
        ...
        if tag_data != self.tag:
            ...
            if len(tag_data) != self.tag_len:
                raise OutOfByteDataError(
                    "Ran out of data when reading tag", offset=start_offset
                )
        ...
        # Decode length
        length, offset = decode_length(
            data, offset, enforce_definite=not self.indefinite_allowed
        )
    ...
    def decode_content(self, data, offset, length):
        ...
        """
        raise NotImplementedError(
            "Type {} does not implement decode_content() method".format(
                type(self).__name__
            )
        )
...
class StandardEncodeMixin(object):
    ...
    def encode(self, data, encoded, values=None):
        ...
        encoded.extend(
            self.tag + encode_length_definite(len(encoded_data)) + encoded_data
        )
...
class PrimitiveOrConstructedType(Type):
    ...
    def __init__(self, name, type_name, number, segment, flags=0):
        super(PrimitiveOrConstructedType, self).__init__(name, type_name, number, flags)
...
class StringType(StandardEncodeMixin, PrimitiveOrConstructedType):
    ...
    def __init__(self, name):
        super(StringType, self).__init__(
            name, self.__class__.__name__, self.TAG, OctetString(name)
        )
...
class MembersType(StandardEncodeMixin, StandardDecodeMixin, Type):
    ...
    def __init__(self, name, tag_name, tag, root_members, additions):
        super(MembersType, self).__init__(name, tag_name, tag, Encoding.CONSTRUCTED)
        self.root_members = root_members
        self.additions = additions

    def set_tag(self, number, flags):
        super(MembersType, self).set_tag(number, flags | Encoding.CONSTRUCTED)
        ...
        try:
            for addition in self.additions:
                ...
                if isinstance(addition, list):
                    ...
                    self.encode_member(addition, data, encoded_addition)
    ...
    def decode_content(self, data, offset, length):
        ...
        offset, out_of_data = self.decode_members(
            self.root_members, data, values, offset, end_offset
        )

        # Decode additions (even if out of data already, so defaults can be added)
        if self.additions:
            offset, out_of_data = self.decode_members(
                flatten(self.additions),
                data,
                values,
                offset,
                end_offset,
    ...
    def decode_members(
        self, members, data, values, offset, end_offset, ignore_missing=False
    ):
        """
        ...
        while True:
            undecoded_members = []
            decode_success = (
                False  # Whether at least one member was successfully decoded
            )
...
class ArrayType(StandardEncodeMixin, StandardDecodeMixin, Type):
    ...
    def __init__(self, name, tag_name, tag, element_type):
        super(ArrayType, self).__init__(name, tag_name, tag, Encoding.CONSTRUCTED)
        self.element_type = element_type

    def set_tag(self, number, flags):
        super(ArrayType, self).set_tag(number, flags | Encoding.CONSTRUCTED)
...
class Boolean(StandardEncodeMixin, StandardDecodeMixin, Type):
    ...
    def decode_content(self, data, offset, length):
        if length != 1:
            raise DecodeError(
                "Expected BOOLEAN contents length 1, but got {}.".format(length),
                offset=offset - 1,
            )
...
class Integer(StandardEncodeMixin, StandardDecodeMixin, Type):
    ...
    def decode_content(self, data, offset, length):
        end_offset = offset + length
        return (
            int.from_bytes(data[offset:end_offset], byteorder="big", signed=True),
            end_offset,
        )
...
class BitString(StandardEncodeMixin, PrimitiveOrConstructedType):
    def __init__(self, name, has_named_bits):
        super(BitString, self).__init__(name, "BIT STRING", Tag.BIT_STRING, self)
...
class OctetString(StandardEncodeMixin, PrimitiveOrConstructedType):
    def __init__(self, name):
        super(OctetString, self).__init__(name, "OCTET STRING", Tag.OCTET_STRING, self)
...
class ObjectIdentifier(StandardEncodeMixin, StandardDecodeMixin, Type):
    def __init__(self, name):
        super(ObjectIdentifier, self).__init__(
            name, "OBJECT IDENTIFIER", Tag.OBJECT_IDENTIFIER
        )
...
class Enumerated(StandardEncodeMixin, StandardDecodeMixin, Type):
    def __init__(self, name, values, numeric):
        super(Enumerated, self).__init__(name, "ENUMERATED", Tag.ENUMERATED)
        if numeric:
...
class Sequence(MembersType):
    def __init__(self, name, root_members, additions):
        super(Sequence, self).__init__(
            name, "SEQUENCE", Tag.SEQUENCE, root_members, additions
        )
...
class SequenceOf(ArrayType):
    def __init__(self, name, element_type):
        super(SequenceOf, self).__init__(
            name, "SEQUENCE OF", Tag.SEQUENCE, element_type
        )


class Set(MembersType):
    def __init__(self, name, root_members, additions):
        super(Set, self).__init__(name, "SET", Tag.SET, root_members, additions)


class SetOf(ArrayType):
    def __init__(self, name, element_type):
        super(SetOf, self).__init__(name, "SET OF", Tag.SET, element_type)
...
class Choice(Type):
    ...
    def __repr__(self):
        return "Choice({}, [{}])".format(
            self.name, ", ".join([repr(member) for member in self.members])
        )
...
class UTCTime(StandardEncodeMixin, StandardDecodeMixin, Type):
    def __init__(self, name):
        super(UTCTime, self).__init__(name, "UTCTime", Tag.UTC_TIME)
...
class GeneralizedTime(StandardEncodeMixin, StandardDecodeMixin, Type):
    def __init__(self, name):
        super(GeneralizedTime, self).__init__(
            name, "GeneralizedTime", Tag.GENERALIZED_TIME
        )
...
class TimeOfDay(StandardEncodeMixin, StandardDecodeMixin, Type):
    def __init__(self, name):
        super(TimeOfDay, self).__init__(name, "TIME-OF-DAY", Tag.TIME_OF_DAY)
...
class DateTime(StandardEncodeMixin, StandardDecodeMixin, Type):
    def __init__(self, name):
        super(DateTime, self).__init__(name, "DATE-TIME", Tag.DATE_TIME)

    def encode_content(self, data, values=None):
        return "{:04d}{:02d}{:02d}{:02d}{:02d}{:02d}".format(*data.timetuple()).encode(
            "ascii"
        )
...
class AnyDefinedBy(Type):
    def __init__(self, name, type_member, choices):
        super(AnyDefinedBy, self).__init__(name, "ANY DEFINED BY", None, None)
    ...
    def encode(self, data, encoded, values):
        if self.choices:
            try:
                ...
                raise EncodeError(
                    "Bad AnyDefinedBy choice {}.".format(values[self.type_member])
                )
        else:
            encoded.extend(data)
    ...
    def decode(self, data, offset, values):
        ...
        if self.choices:
            try:
                return self.choices[values[self.type_member]].decode(data, offset)
            except KeyError:
                raise DecodeError(
                    "Bad AnyDefinedBy choice {}.".format(values[self.type_member]),
                    offset=offset,
...
class ExplicitTag(StandardEncodeMixin, StandardDecodeMixin, Type):
    ...
    def set_tag(self, number, flags):
        super(ExplicitTag, self).set_tag(number, flags | Encoding.CONSTRUCTED)
    ...
    def decode_content(self, data, offset, length):
        ...
        if length is None:
            if not detect_end_of_contents_tag(data, end_offset):
                raise NoEndOfContentsTagError(
                    "Expected end-of-contents tag.", offset=end_offset, location=self
                )
            end_offset += 2
...
class Compiler(compiler.Compiler):
    ...
    def compile_implicit_type(self, name, type_descriptor, module_name):
        ...
        if type_name == "SEQUENCE":
            ...
            compiled = SequenceOf(
                name, self.compile_type("", type_descriptor["element"], module_name)
            )
        elif type_name == "SET":
            compiled = Set(
                name,
            ...
            for key, value in type_descriptor["choices"].items():
                choices[key] = self.compile_type(key, value, module_name)

            compiled = AnyDefinedBy(name, type_descriptor["value"], choices)
        elif type_name == "NULL":
            compiled = Null(name)
    ...
    def compile_type(self, name, type_descriptor, module_name):
        ...
        compiled = self.compile_implicit_type(name, type_descriptor, module_name)
    ...
    def compile_members(self, members, module_name, sort_by_tag=False):
        ...
        for member in members:
            if member == EXTENSION_MARKER:
                ...
                self.compile_extension_member(member, module_name, additions)
            else:
    ...
    def compile_extension_member(self, member, module_name, additions):

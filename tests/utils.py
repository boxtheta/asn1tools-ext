...
def format_encoded(encoded):
    ...
    binstring = " ".join([binstring[i : i + 8] for i in range(0, len(binstring), 8)])
...
class Asn1ToolsBaseTest(unittest.TestCase):
    def assert_encode_decode(
        self, specification, type_name, decoded_message, encoded_message
    ):
        try:
    ...
    def assert_encode_decode_string(
        self, specification, type_name, decoded_message, encoded_message, indent=None
    ):
        try:
            encoded = specification.encode(type_name, decoded_message, indent=indent)
        except:
            print("Failed to encode type with name {}.".format(type_name))
            raise

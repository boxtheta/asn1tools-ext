...
class Asn1ToolsBerTest(Asn1ToolsBaseTest):
    ...
    def test_boolean(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= BOOLEAN END"
        )

        datas = [("A", True, b"\x01\x01\xff"), ("A", False, b"\x01\x01\x00")]
    ...
    def test_integer(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= INTEGER END"
        )
    ...
    def test_real(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= REAL END"
        )
        ...
        datas = [
            ...
            ("A", 1.1, b"\x09\x09\x80\xcd\x08\xcc\xcc\xcc\xcc\xcc\xcd"),
            ("A", 1234.5678, b"\x09\x09\x80\xd6\x13\x4a\x45\x6d\x5c\xfa\xad"),
        ...
        self.assertEqual(
            str(cm.exception), "A: Unsupported special REAL control word 0x44."
        )
        ...
        self.assertEqual(
            str(cm.exception), "A: Unsupported binary REAL control word 0x82."
        )

        # Decode 100.0 in decimal form (1.e2).
        self.assertEqual(foo.decode("A", b"\x09\x05\x03\x31\x2e\x45\x32"), 100.0)
        # Decode 100.0 in decimal form with comma (100,0).
        self.assertEqual(foo.decode("A", b"\x09\x06\x02\x31\x30\x30\x2c\x30"), 100.0)
    ...
    def test_bit_string(self):
        ...
        datas = [
            ...
            ("D", {"a": (b"\x00", 2), "b": (b"\x00", 2)}, b"\x30\x00"),
        ]
    ...
    def test_external_explicit_tags(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS ::= BEGIN A ::= EXTERNAL B ::= [0] EXTERNAL END"
        )

        datas = [
            ("A", {"encoding": ("octet-aligned", b"\x12")}, b"\x28\x03\x81\x01\x12"),
            (
                "A",
                {"data-value-descriptor": "12", "encoding": ("octet-aligned", b"\x34")},
    ...
    def test_external_implicit_tags(self):
        ...
        datas = [
            ...
            ("B", {"encoding": ("octet-aligned", b"\x12")}, b"\xa0\x03\x81\x01\x12"),
            (
                "B",
                {"data-value-descriptor": "12", "encoding": ("octet-aligned", b"\x34")},
    ...
    def test_sequence(self):
        ...
        datas = [
            ...
            ("P", {"a": True, "b": False}, b"\x30\x06\x80\x01\xff\x81\x01\x00"),
            ("D", {"a": True, "b": True}, b"\x30\x06\x80\x01\xff\x81\x01\xff"),
            ("E", {"a": True, "b": True}, b"\x30\x06\x80\x01\xff\x81\x01\xff"),
            ("F", {"a": True, "c": True}, b"\x30\x06\x80\x01\xff\x82\x01\xff"),
            ("G", {"a": True, "d": True}, b"\x30\x06\x80\x01\xff\x83\x01\xff"),
            ("I", {"a": True, "b": True}, b"\x30\x06\x80\x01\xff\x81\x01\xff"),
            ("J", {"a": True, "b": True}, b"\x30\x06\x80\x01\xff\x81\x01\xff"),
            ("K", {"a": True, "b": True}, b"\x30\x06\x80\x01\xff\x81\x01\xff"),
            (
                "F",
            ...
            ("U", {"a": [{"a": []}]}, b"\x30\x06\xa0\x04\x30\x02\xa0\x00"),
            (
                "V",
                {"a": {}, "b": {}, "c": {"a": {}}},
        ...
        self.assertEqual(
            foo.decode("P", b"\x30\x03\x80\x01\xff"), {"a": True, "b": True}
        )

        # Decode D as C. Extension addition "a.b" should be skipped.
        self.assertEqual(
            foo.decode("C", b"\x30\x06\x80\x01\xff\x81\x01\xff"), {"a": True}
        )

        # Decode R as Q. Extension addition "a.b" should be skipped.
        self.assertEqual(
            foo.decode("Q", b"\x30\x0b\xa0\x06\x80\x01\xff\x81\x01\xff\x81\x01\x64"),
            {"a": {"a": True}, "b": 100},
        )
        ...
        self.assertEqual(
            foo.decode(
                "W",
                b"\x30\x80\x81\x01\x10\x82\x01\x61\xa3\x80\x84\x01\x02\x00\x00\x00\x00",
            ),
            {"a": 16, "b": b"a", "c": {"a": 2}},
        )
        ...
        with self.assertRaises(asn1tools.DecodeError) as cm:
            foo.decode(
                "W", b"\x30\x80\x81\x01\x10\x82\x01\x61\xa3\x80\x84\x01\x02\x00\x00"
            )
        ...
        self.assertEqual(str(cm.exception), "C: Sequence member 'a' not found in {}.")

    def test_sequence_of(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= SEQUENCE OF INTEGER END",
            "ber",
        )
    ...
    def test_set(self):
        ...
        self.assertEqual(
            foo.decode("A", b"\x31\x80\x80\x01\x03\x81\x01\x00\x00\x00"),
            {"a": 3, "b": 0},
        )
        self.assertEqual(
            foo.decode(
                "C",
                b"\x31\x80\xa0\x80\x80\x01\x03\x81\x01\x04\x00\x00\x81\x01\x12\x00\x00",
            ),
            {"a": {"a": 3, "b": 4}, "b": b"\x12"},
        )
        # Test error when EOC tag is missing
        with self.assertRaises(asn1tools.DecodeError) as cm:
            foo.decode(
                "C", b"\x31\x80\xa0\x80\x80\x01\x03\x81\x01\x04\x81\x01\x12\x00\x00"
            )
        ...
        self.assertEqual(
            foo.decode("A", b"\x31\x06\x81\x01\x04\x80\x01\x03"), {"a": 3, "b": 4}
        )
        self.assertEqual(
            foo.decode("A", b"\x31\x80\x81\x01\x00\x80\x01\x03\x00\x00"),
            {"a": 3, "b": 0},
        )
    ...
    def test_choice(self):
        ...
        self.assertEqual(
            foo.decode("M", b"\xa0\x80\x80\x01\xff\x00\x00"), ("a", ("b", True))
        )
    ...
    def test_choice_implicit_tags(self):
        ...
        self.assertEqual(
            foo.decode("K", b"\xa3\x80\xa4\x80\x01\x01\x00\x00\x00\x00\x00"),
            ("a", ("a", False)),
        )
    ...
    def test_utf8_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= UTF8String END", "ber"
        )

        datas = [
            ("A", "bar", b"\x0c\x03\x62\x61\x72"),
            ("A", "a\u1010c", b"\x0c\x05\x61\xe1\x80\x90\x63"),
            ("A", "f → ∝", b"\x0c\x09\x66\x20\xe2\x86\x92\x20\xe2\x88\x9d"),
        ]
    ...
    def test_bmp_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS IMPLICIT TAGS ::= BEGIN A ::= BMPString END"
        )

        datas = [
            ("A", "hello", b"\x1e\x0a\x00\x68\x00\x65\x00\x6c\x00\x6c\x00\x6f"),
            ("A", "\u0102", b"\x1e\x02\x01\x02"),
        ]
    ...
    def test_graphic_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= GraphicString END"
        )

        datas = [("A", "f", b"\x19\x01\x66")]
    ...
    def test_universal_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= UniversalString END"
        )

        datas = [
            ("A", "bar", b"\x1c\x0c\x00\x00\x00\x62\x00\x00\x00\x61\x00\x00\x00\x72"),
            ("A", "åäö", b"\x1c\x0c\x00\x00\x00\xe5\x00\x00\x00\xe4\x00\x00\x00\xf6"),
        ]
    ...
    def test_utc_time(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS IMPLICIT TAGS ::= BEGIN A ::= UTCTime END"
        )
        ...
        datas = [
            ...
            ("A", ut2dt("010203040506Z"), b"\x17\x0d010203040506Z"),
        ]
    ...
    def test_utc_time_explicit_tags(self):
        ...
        self.assert_encode_decode(
            foo, "Foo", ut2dt("121001230001Z"), b"\xa2\x0f\x17\x0d121001230001Z"
        )

    def test_generalized_time(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= GeneralizedTime END"
        )
        ...
        datas = [
            ...
            (
                "A",
                datetime(
                    2000, 12, 31, 23, 59, 59, 999000, timezone(timedelta(hours=0))
                ),
    ...
    def test_decode_length(self):
        ...
        datas = [b"\x30", b"", b"\x30\x84\x00\x00\x00"]
    ...
    def test_rrc_8_6_0(self):
        ...
        decoded = {
            "message": (
                ...
                (
                    "paging",
                    {"systemInfoModification": "true", "nonCriticalExtension": {}},
                ),
            )
        }
        ...
        decoded = {
            "message": {
                "dl-Bandwidth": "n6",
                "phich-Config": {"phich-Duration": "normal", "phich-Resource": "half"},
                        ...
                        "criticalExtensions": (
                            "systemInformation-r8",
                            {
                                "sib-TypeAndInfo": [
                                    ...
                                    ("sib6", {"t-ReselectionUTRA": 3}),
                                    ("sib7", {"t-ReselectionGERAN": 3}),
                                    (
                                        "sib8",
                                        {
                                            "parameters1XRTT": {
    ...
    def test_rfc1157(self):
        rfc1157 = asn1tools.compile_files(
            ["tests/files/ietf/rfc1155.asn", "tests/files/ietf/rfc1157.asn"]
        )
                    ...
                    "variable-bindings": [
                        ...
                        },
                        {
                            ...
                            "value": (
                                ...
                                (
                                    "string",
                                    (b"\x31\x37\x32\x2e\x33\x31\x2e\x31\x39\x2e\x32"),
                                ),
                            ),
                        },
                    ],
        ...
        decoded = {
            ...
            "data": (
                "set-request",
                {
                    ...
                    "error-index": 0,
                    "variable-bindings": [
                        {"name": "1.3.6.1.999.1.1", "value": ("simple", ("number", 1))},
                        {
                            "name": "1.3.6.1.999.2.1",
                            "value": ("simple", ("string", b"f00")),
                        },
                        {
                            "name": "1.3.6.1.999.3.1",
                            "value": (
                                "application-wide",
                                ("address", ("internet", b"\xc0\xa8\x01\x01")),
                            ),
                        },
                        {
                            "name": "1.3.6.1.999.4.1",
                            "value": ("simple", ("object", "1.2.3.444.555")),
                        },
                        {"name": "1.3.6.1.999.1.2", "value": ("simple", ("number", 1))},
                        {
                        ...
                        {"name": "1.3.6.1.999.1.3", "value": ("simple", ("number", 1))},
                        {
            ...
            "data": (
                "set-request",
                {
                    ...
                    "variable-bindings": [
                        ...
                        {
                            "name": "1.3.6.1.999.3.1",
                            "value": ("application-wide", ("counter", 0)),
                        },
                        {
                            "name": "1.3.6.1.999.3.1",
                            "value": ("application-wide", ("gauge", 4294967295)),
                        },
                        {
                            "name": "1.3.6.1.999.3.1",
                            "value": ("application-wide", ("ticks", 88)),
                        },
                        {
                            "name": "1.3.6.1.999.3.1",
                            "value": (
    ...
    def test_rfc4511(self):
        ...
        decoded = {
            "controls": [
                ...
                {"controlType": b"2.16.840.1.113730.3.4.2", "criticality": False},
            ],
            "messageID": 5,
            "protocolOp": (
                "searchRequest",
        ...
        encoded = b"\x30\x0c\x02\x01\x02\x65\x07\x0a\x01\x20\x04\x00\x04\x00"
        ...
        decoded = {
            ...
            "protocolOp": (
                "bindResponse",
                {"resultCode": "success", "matchedDN": b"", "diagnosticMessage": b""},
            ),
        }

        encoded = b"\x30\x0c\x02\x01\x01\x61\x07\x0a\x01\x00\x04\x00\x04\x00"
        ...
        decoded = {
            "tbsCertificate": {
                ...
                "issuer": (
                    "rdnSequence",
                    [
                        [{"type": "2.5.4.6", "value": b"\x13\x02\x4a\x50"}],
                        [{"type": "2.5.4.8", "value": b"\x13\x05\x54\x6f\x6b\x79\x6f"}],
                        [
                            {
                                "type": "2.5.4.7",
                        ...
                        [
                            {
                                "type": "2.5.4.11",
                                "value": (
                                    b"\x13\x0f\x57\x65\x62\x43\x65\x72\x74\x20\x53"
                        ...
                        [
                            {
                                "type": "2.5.4.3",
                                "value": (
                                    b"\x13\x0f\x46\x72\x61\x6e\x6b\x34\x44\x44\x20"
                ...
                "subject": (
                    ...
                    [
                        [{"type": "2.5.4.6", "value": b"\x13\x02\x4a\x50"}],
                        [{"type": "2.5.4.8", "value": b"\x0c\x05\x54\x6f\x6b\x79\x6f"}],
                        [
                            {
                                "type": "2.5.4.10",
                        ...
                        [
                            {
                                "type": "2.5.4.3",
                                "value": (
                                    b"\x0c\x0f\x77\x77\x77\x2e\x65\x78\x61\x6d\x70"
                ...
                "subjectPublicKeyInfo": {
                    ...
                    "subjectPublicKey": (
                        b"0H\x02A\x00\x9b\xfcf\x90y\x84B\xbb"
    ...
    def test_rfc5280_modified(self):
        ...
        rfc5280 = asn1tools.compile_dict(
            deepcopy(RFC5280_MODIFIED), any_defined_by_choices=any_defined_by_choices
        )
        ...
        decoded = {
            "tbsCertificate": {
                "version": "v1",
                "serialNumber": 3578,
                "signature": {"algorithm": "1.2.840.113549.1.1.5", "parameters": None},
                "issuer": (
                    "rdnSequence",
                    [
                        [{"type": "2.5.4.6", "value": "JP"}],
                        [{"type": "2.5.4.8", "value": ("printableString", "Tokyo")}],
                ...
                "subject": (
                    ...
                    [
                        ...
                        [{"type": "2.5.4.10", "value": ("utf8String", "Frank4DD")}],
                        [
                            {
                                "type": "2.5.4.3",
        ...
        # Explicit tagging.
        decoded = ("psap-address", {"pSelector": b"\x12", "nAddresses": [b"\x34"]})
    ...
    def test_all_types(self):
        ...
        self.assertEqual(
            all_types.decode("Sequence12", b"\x30\x04\x30\x02\x30\x00"), {"a": [{}]}
        )

    def test_all_types_automatic_tags(self):
        all_types = asn1tools.compile_files("tests/files/all_types_automatic_tags.asn")
    ...
    def test_module_tags_explicit(self):
        ...
        datas = [
            ...
            (
                "S3",
                {"a": 1, "b": {"a": 3}, "c": ("a", True)},
                b"\x30\x0f\x02\x01\x01\xa2\x05\x30\x03\x02\x01\x03\xa3\x03\x01\x01\xff",
            ),
            (
                "S4",
    ...
    def test_repr_all_types(self):
        ...
        self.assertEqual(
            repr(all_types.types["Octetstring"]), "OctetString(Octetstring)"
        )
        self.assertEqual(repr(all_types.types["Null"]), "Null(Null)")
        self.assertEqual(
            repr(all_types.types["Objectidentifier"]),
        ...
        self.assertEqual(
            repr(all_types.types["Sequence2"]), "Sequence(Sequence2, [Integer(a)])"
        )
        self.assertEqual(repr(all_types.types["Set2"]), "Set(Set2, [Integer(a)])")
        self.assertEqual(
            repr(all_types.types["Numericstring"]), "NumericString(Numericstring)"
        )
        self.assertEqual(
            repr(all_types.types["Printablestring"]), "PrintableString(Printablestring)"
        )
        self.assertEqual(repr(all_types.types["Ia5string"]), "IA5String(Ia5string)")
        self.assertEqual(
            repr(all_types.types["Universalstring"]), "UniversalString(Universalstring)"
        )
        self.assertEqual(
            repr(all_types.types["Visiblestring"]), "VisibleString(Visiblestring)"
        )
        self.assertEqual(
            repr(all_types.types["Generalstring"]), "GeneralString(Generalstring)"
        )
        self.assertEqual(repr(all_types.types["Bmpstring"]), "BMPString(Bmpstring)")
        self.assertEqual(
            repr(all_types.types["Teletexstring"]), "TeletexString(Teletexstring)"
        )
        self.assertEqual(
            repr(all_types.types["Graphicstring"]), "GraphicString(Graphicstring)"
        )
        self.assertEqual(repr(all_types.types["Utctime"]), "UTCTime(Utctime)")
        self.assertEqual(
            repr(all_types.types["SequenceOf"]), "SequenceOf(SequenceOf, Integer())"
        )
        self.assertEqual(repr(all_types.types["SetOf"]), "SetOf(SetOf, Integer())")
        self.assertEqual(
            repr(all_types.types["GeneralizedTime1"]),
            "GeneralizedTime(GeneralizedTime1)",
        )
        self.assertEqual(
            repr(all_types.types["Choice"]), "Choice(Choice, [Integer(a)])"
        )
    ...
    def test_nested_explicit_tags(self):
        ...
        decoded = {"outernumber": 23, "inner": {"innernumber": 42}}
    ...
    def test_zforce(self):
        ...
        decoded = (
            ...
            {
                "enable": {"reset": None},
                "openShort": {"openShortInfo": [], "errorCount": 34},
            },
        )
        encoded = b"\xef\x0b\x65\x02\x82\x00\x6a\x05\xa0\x00\x81\x01\x22"
        ...
        decoded = (
            "notification",
            {
                "notificationMessage": (
                    "ledLevels",
                    [("uint8", b"\x55\x44\x33\x22"), ("uint16", b"\x01\x02")],
                ),
            ...
            "request",
            {
                ...
                "errorThresholds": {
                    "asicsThresholds": [
                        {
                            "asicIdentifier": -256,
                            "irLedOpenThresholds": {"low": -4500, "high": 100000},
                        }
                    ]
                },
            },
    ...
    def test_bar(self):
        ...
        decoded = {
            "headerOnly": True,
            "lock": False,
            "acceptTypes": {"standardTypes": [(b"\x40", 4), (b"\x80", 4)]},
        ...
        # Message 2.
        decoded = {"headerOnly": False, "lock": False, "url": b"0"}
    ...
    def test_any_defined_by_integer(self):
        ...
        any_defined_by_choices = {("Foo", "Fie", "fum"): {0: "NULL", 1: "INTEGER"}}

        foo = asn1tools.compile_string(
            spec, any_defined_by_choices=any_defined_by_choices
        )

        # Message 1.
        decoded = {"bar": 0, "fum": None}
        ...
        self.assertEqual(
            str(cm.exception), "Fie.fum: Bad AnyDefinedBy choice 2. (At offset: 5)"
        )
    ...
    def test_any_defined_by_object_identifier(self):
        ...
        any_defined_by_choices = {
            ("Foo", "Fie", "fum"): {"1.3.6.2": "IA5String", "1.3.1000.7": "BOOLEAN"}
        }
        ...
        # Message 2.
        decoded = {"bar": "1.3.1000.7", "fum": True}
        ...
        # Message 3, key not found.
        decoded = {"bar": "1.3.1000.8", "fum": True}
        ...
        self.assertEqual(
            str(cm.exception), "Fie.fum: Bad AnyDefinedBy choice 1.3.1000.8."
        )
    ...
    def test_all_types_constructed_definite_length(self):
        ...
        datas = [
            ...
            ("Bmpstring", "fie", b"\x3e\x0a\x04\x02\x00\x66\x04\x04\x00\x69\x00\x65"),
    ...
    def test_long_tag(self):
        ...
        datas = [("A", 1, b"\x9f\x1f\x01\x01"), ("B", 1, b"\x9f\x83\x74\x01\x01")]
    ...
    def test_decode_with_length(self):
        ...
        self.assertEqual(
            foo.decode_with_length("A", b"\x02\x03\x00\x80\x00"), (32768, 5)
        )
        # Test decoding indefinite length CHOICE
        self.assertEqual(
            foo.decode_with_length(
                "K", b"\xa3\x80\xa4\x80\x01\x01\x00\x00\x00\x00\x00"
            ),
            (("a", ("a", False)), 11),
        )
        # Test decoding definite length CHOICE
        self.assertEqual(
            foo.decode_with_length("K", b"\xa3\x05\xa5\x03\x02\x01\x02"),
            (("b", ("b", 2)), 7),
        )
        # Test indefinite length set with end-of-contents tags
        self.assertEqual(

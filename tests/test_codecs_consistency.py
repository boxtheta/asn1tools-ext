...
class Asn1ToolsCodecsConsistencyTest(Asn1ToolsBaseTest):
    ...
    def encode_decode_all_codecs(self, type_spec, values, numeric_enums=False):
        spec = (
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= " + type_spec + " " + "END"
        )
        ...
        for codec in CODECS:
            foos.append(
                asn1tools.compile_string(spec, codec, numeric_enums=numeric_enums)
            )

        gser = asn1tools.compile_string(spec, "gser", numeric_enums=numeric_enums)
    ...
    def encode_decode_codec(self, spec, codec, type_name, decoded, encoded):
        encoded_message = spec.encode(type_name, decoded, check_constraints=True)
        ...
        decoded_message = spec.decode(type_name, encoded, check_constraints=True)
    ...
    def encode_decode_codecs(self, type_spec, decoded, encoded):
        ...
        for codec, encoded_message in zip(ALL_CODECS, encoded):
            ...
            self.encode_decode_codec(foo, codec, "A", decoded, encoded_message)

    def encode_decode_codecs_file(self, filename, type_name, decoded, encoded):
        for codec, encoded_message in zip(ALL_CODECS, encoded):
            foo = asn1tools.compile_files(filename, codec)
            self.encode_decode_codec(foo, codec, type_name, decoded, encoded_message)
    ...
    def decode_codecs(self, type_spec, decoded, encoded, numeric_enums=False):
        ...
        for codec, encoded_message in zip(CODECS, encoded):
            ...
            decoded_message = foo.decode("A", encoded_message, check_constraints=True)
            self.assertEqual(decoded_message, decoded)
    ...
    def test_integer_versions(self):
        ...
        self.decode_codecs("INTEGER (1, ..., 2)", decoded_v2, encoded_v2)

        self.decode_codecs("INTEGER (1, ...)", decoded_v1, encoded_v2)
    ...
    def test_bit_string(self):
        self.encode_decode_all_codecs("BIT STRING", [(b"\x58", 5), (b"\x58\x80", 9)])
    ...
    def test_enumerated_numeric(self):
        self.encode_decode_all_codecs(
            "ENUMERATED { a(0), b(5) }", [0, 5], numeric_enums=True
        )
        ...
        self.decode_codecs(
            "SEQUENCE { e ENUMERATED { a(0), b(5) } DEFAULT b }",
            decoded,
            encoded_messages,
            numeric_enums=True,
        )
    ...
    def test_enumerated_versions(self):
        ...
        self.decode_codecs("ENUMERATED {a, b, ..., c}", decoded_v2, encoded_v2)

        self.decode_codecs("ENUMERATED {a, b, ...}", decoded_v1, encoded_v2)
    ...
    def test_choice_versions(self):
        ...
        decoded_v1 = {"a": (None, None), "d": True}

        decoded_v2 = {"a": ("c", None), "d": True}
        ...
        self.decode_codecs(
            "SEQUENCE {   a CHOICE {b NULL, ..., c NULL},   d BOOLEAN }",
            decoded_v2,
            encoded_v2,
        )

        self.decode_codecs(
            "SEQUENCE {   a CHOICE {b NULL, ...},   d BOOLEAN }", decoded_v1, encoded_v2
        )

    def test_utf8_string(self):
        self.encode_decode_all_codecs("UTF8String", ["hi"])

    def test_numeric_string(self):
        self.encode_decode_all_codecs("NumericString", ["123"])

    def test_printable_string(self):
        self.encode_decode_all_codecs("PrintableString", ["hi"])

    def test_ia5_string(self):
        self.encode_decode_all_codecs("IA5String", ["hi"])

    def test_visible_string(self):
        self.encode_decode_all_codecs("VisibleString", ["hi"])

    def test_general_string(self):
        self.encode_decode_all_codecs("GeneralString", ["hi"])

    def test_bmp_string(self):
        self.encode_decode_all_codecs("BMPString", ["hi"])

    def test_graphic_string(self):
        self.encode_decode_all_codecs("GraphicString", ["hi"])

    def test_teletex_string(self):
        self.encode_decode_all_codecs("TeletexString", ["hi"])

    def test_universal_string(self):
        self.encode_decode_all_codecs("UniversalString", ["hi"])

    def test_utc_time(self):
        self.encode_decode_all_codecs("UTCTime", [datetime(2020, 3, 12)])

    def test_generalized_time(self):
        self.encode_decode_all_codecs("GeneralizedTime", [datetime(2021, 3, 12)])
    ...
    def test_error_location(self):
        ...
        for codec in ALL_CODECS:
            ...
            with self.assertRaises(asn1tools.EncodeError) as cm:
                foo.encode("A", {"a": {"b": ("c", {})}})
            self.assertEqual(
                str(cm.exception), "A.a.b.c: Sequence member 'd' not found in {}."
            )

    def test_recursive(self):
        spec = "SEQUENCE {   a B } B ::= CHOICE {   b A,   c NULL } "

        self.encode_decode_all_codecs(spec, [{"a": ("b", {"a": ("c", None)})}])

    def test_with_components(self):
        decoded = {"a": 1, "b": {"c": [("f", True)], "d": 2}}
        ...
        self.encode_decode_codecs_file(
            "tests/files/with_components.asn", "Foo", decoded, encoded
        )
    ...
    def test_integer_min_max(self):
        ...
        self.encode_decode_codecs("INTEGER (MIN..MAX)", decoded, encoded)
    ...
    def test_enumerated_all_except(self):
        ...
        self.encode_decode_codecs(
            "ENUMERATED {a, b, c, d, e} (ALL EXCEPT b)", decoded, encoded
        )
    ...
    def test_date_time(self):
        decoded = [datetime(1985, 4, 12, 15, 27, 46), datetime(1582, 2, 3, 4, 5, 6)]

        encoded = [
            [
                b"\x1f\x21\x0e\x31\x39\x38\x35\x30\x34\x31\x32\x31\x35\x32\x37\x34\x36",
                b"\x1f\x21\x0e\x31\x39\x38\x35\x30\x34\x31\x32\x31\x35\x32\x37\x34\x36",
        ...
        for decoded_message, encoded_message in zip(decoded, encoded):
            self.encode_decode_codecs("DATE-TIME", decoded_message, encoded_message)
    ...
    def test_named_numbers(self):
        ...
        for type_name, decoded, encoded in datas:
            self.encode_decode_codecs_file(
                "tests/files/named_numbers.asn", type_name, decoded, encoded
            )
    ...
    def test_named_numbers_errors(self):
        datas = [
            ...
            ("A", -2, "A: Expected an integer between -1 and 1, but got -2."),
            ("A", 2, "A: Expected an integer between -1 and 1, but got 2."),
            ("B", 1, "B: Expected an integer between 2 and 2, but got 1."),
            ("C", -1, "C: Expected an integer between 0 and 1, but got -1."),
            ("C", 2, "C: Expected an integer between 0 and 1, but got 2."),
        ]

        for codec in ALL_CODECS:
            foo = asn1tools.compile_files("tests/files/named_numbers.asn", codec)
        ...
        for codec in CODECS:
            specs.append(
                asn1tools.compile_files(
                    "examples/programming_types/programming_types.asn", codec
                )
            )
    ...
    def test_c_source(self):
        ...
        decoded = [
            {
                "a": {"b": ("c", 0), "e": [None, None, None], "f": None},
                "g": {"h": "j", "l": b"\x54\x55"},
                "m": {
                    "n": False,
                    "o": 2,
        ...
        decoded = [
            {
                "a": {"b": ("d", False), "e": [None, None, None], "f": None},
                "g": {"h": "k", "l": b"\x54"},
                "m": {"o": 3, "p": {"q": 5 * b"\x03"}, "s": False},
            }
        ]
        ...
        encoded_messages = [b"\x05\x00", b"\x05\x00", b"null", b"", b"", b"", b"<H />"]
        ...
        # AE
        decoded = {"a": False, "b": True, "c": False}
    ...
    def test_parameterization(self):
        ...
        for codec in CODECS:
            specs.append(asn1tools.compile_dict(deepcopy(PARAMETERIZATION), codec))
        ...
        for spec, codec, encoded in zip(specs, CODECS, encoded_messages):
            self.encode_decode_codec(spec, codec, "B-BooleanInteger", decoded, encoded)

        # D.
        decoded = {"a": ("a", {"a": True}), "b": ("c", {"a": {"a": None, "b": 5}})}
        ...
        encoded_messages = [
            ...
            b"\x28\x01\x05",
            b"\x28\x08\x28",
            b"<D><a><a><a><true /></a></a></a><b><c><a><a /><b>5</b></a></c></b></D>",
        ]

        for spec, codec, encoded in zip(specs, CODECS, encoded_messages):
            self.encode_decode_codec(spec, codec, "D", decoded, encoded)

        decoded = {"a": ("a", {"a": True}), "b": ("d", {"a": None, "b": 5})}
        ...
        decoded = {"a": ("b", {"a": {"a": True}}), "b": ("d", {"a": None, "b": 5})}
        ...
        encoded_messages = [
            b"\x30\x10\xa1\x0e\xa0\x0c\x30\x0a\xa0\x03\x02\x01\x02\xa1\x03\x01\x01\xff",
            b"\x30\x10\xa1\x0e\xa0\x0c\x30\x0a\xa0\x03\x02\x01\x02\xa1\x03\x01\x01\xff",
            b'[{"b": {"c": [{"a": 2, "b": true}]}}]',
            b"\x01\x01\x81\x01\x01\x80\x01\x02\xff",
            b"\x30\x01\x80\x01\x02\x80",
            b"\x30\x18\x08\x14",
            b"<F><b><c><SEQUENCE><a>2</a><b><true /></b></SEQUENCE></c></b></F>",
        ]

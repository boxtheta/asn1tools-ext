...
class Asn1ToolsXerTest(Asn1ToolsBaseTest):
    ...
    def test_real(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= REAL END", "xer"
        )
    ...
    def test_bit_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= BIT STRING END", "xer"
        )
    ...
    def test_octet_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= OCTET STRING END", "xer"
        )
    ...
    def test_object_identifier(self):
        ...
        self.assertEqual(
            str(cm.exception), "A: Expected an OBJECT IDENTIFIER, but got ''."
        )

    def test_external(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= EXTERNAL END", "xer"
        )
    ...
    def test_enumerated(self):
        ...
        self.assertEqual(
            str(cm.exception), "B: Expected enumeration value 'a', but got 'foo'."
        )
        ...
        self.assertEqual(
            str(cm.exception), "B: Expected enumeration value 'a', but got 'bar'."
        )
    ...
    def test_choice(self):
        ...
        self.assertEqual(
            str(cm.exception), "A: Expected choice 'a', 'b' or 'c', but got 'd'."
        )
        ...
        self.assertEqual(
            str(cm.exception), "B: Expected choice 'a', 'b' or 'c', but got 'd'."
        )
    ...
    def test_utf8_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= UTF8String END", "xer"
        )
    ...
    def test_universal_string(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= UniversalString END", "xer"
        )

        datas = [("A", "bar", b"<A>bar</A>")]
    ...
    def test_utc_time(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= UTCTime END", "xer"
        )
    ...
    def test_generalized_time(self):
        foo = asn1tools.compile_string(
            "Foo DEFINITIONS AUTOMATIC TAGS ::= BEGIN A ::= GeneralizedTime END", "xer"
        )
    ...
    def test_any(self):
        foo = asn1tools.compile_string("Foo DEFINITIONS ::= BEGIN A ::= ANY END", "xer")
    ...
    def test_foo(self):
        ...
        encoded_message = b"<Answer><id>1</id><answer><false /></answer></Answer>"
    ...
    def test_repr_all_types(self):
        ...
        self.assertEqual(
            repr(foo.types["Objectidentifier"]), "ObjectIdentifier(Objectidentifier)"
        )
        ...
        self.assertEqual(
            repr(foo.types["Sequence2"]), "Sequence(Sequence2, [Integer(a)])"
        )
        self.assertEqual(repr(foo.types["Set2"]), "Set(Set2, [Integer(a)])")
        self.assertEqual(
            repr(foo.types["Numericstring"]), "NumericString(Numericstring)"
        )
        self.assertEqual(
            repr(foo.types["Printablestring"]), "PrintableString(Printablestring)"
        )
        self.assertEqual(repr(foo.types["Ia5string"]), "IA5String(Ia5string)")
        self.assertEqual(
            repr(foo.types["Universalstring"]), "UniversalString(Universalstring)"
        )
        self.assertEqual(
            repr(foo.types["Visiblestring"]), "VisibleString(Visiblestring)"
        )
        self.assertEqual(
            repr(foo.types["Generalstring"]), "GeneralString(Generalstring)"
        )
        self.assertEqual(repr(foo.types["Bmpstring"]), "BMPString(Bmpstring)")
        self.assertEqual(
            repr(foo.types["Teletexstring"]), "TeletexString(Teletexstring)"
        )
        self.assertEqual(repr(foo.types["Utctime"]), "UTCTime(Utctime)")
        self.assertEqual(
            repr(foo.types["SequenceOf"]), "SequenceOf(SequenceOf, Integer(INTEGER))"
        )
        self.assertEqual(repr(foo.types["SetOf"]), "SetOf(SetOf, Integer(INTEGER))")
        self.assertEqual(
            repr(foo.types["GeneralizedTime1"]), "GeneralizedTime(GeneralizedTime1)"
        )
        self.assertEqual(repr(foo.types["Choice"]), "Choice(Choice, [Integer(a)])")
        self.assertEqual(repr(foo.types["Any"]), "Any(Any)")
        self.assertEqual(
            repr(foo.types["Sequence12"]),
            "Sequence(Sequence12, [SequenceOf(a, Recursive(Sequence12))])",
        )

    def test_all_types_automatic_tags(self):
        foo = asn1tools.compile_files("tests/files/all_types_automatic_tags.asn", "xer")
    ...
    def test_indent(self):
        ...
        for type_name, decoded, encoded in datas:
            self.assert_encode_decode_string(foo, type_name, decoded, encoded, indent=4)
    ...
    def test_issue_34(self):
        ...
        self.assert_encode_decode_string(foo, "A", decoded, encoded)
